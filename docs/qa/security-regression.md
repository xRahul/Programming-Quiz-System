...
## Open defect found by the T8 gap sweep

| # | Finding | Status | Evidence |
|---|---|---|---|
| 20 | **resetTables half-resets and locks out the database.** admin.php's handler runs bare `TRUNCATE TABLE` in parent-before-child order (admins, answers, questions, quizes, quiz_takers). Under the FK schema shipped by migration runner (fk_answers_question/fk_answers_quiz/fk_questions_quiz/fk_takers_quiz), `TRUNCATE TABLE questions` aborts with MariaDB error 1701 even though answers is empty — TRUNCATE fails whenever a FK references the table. Sequence dies as a blank `HTTP/1.0 500` AFTER admins was truncated but BEFORE the default-admin INSERT: verified repro leaves `admins=0, quizes=2, questions=31` — no admin can ever log in again on that DB without manual SQL. Legacy pre-FK schema never hit this because unconstrained InnoDB allowed the truncates. | OPEN — reported per T8 brief ("do not fix silently"); guarded probe committed | Repro (scratch DB `debug_test_p8_probe`, php -S worker): authed POST `admin.php` with `resetTables=yes` + valid CSRF token → `HTTP/1.0 500 Internal Server Error`, empty body; DB state after: admins empty, remaining tables untouched, no reseed. Protecting probe: `ResetTablesReseedTest::testResetTablesTruncatesAndReseedsBcryptAdmin` — currently SKIPs with a pointer to this row; its full assertion set (exactly-one bcrypt-verifiable admin/'12345', all content tables truncated, audited) activates automatically once the handler is repaired (e.g. ordered `DELETE FROM` inside a transaction or `SET FOREIGN_KEY_CHECKS=0` window). |
